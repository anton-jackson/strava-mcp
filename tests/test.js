...
const strava_v3_1 = __importDefault(require("strava-v3"));
...
class StravaClient {
    constructor(accessToken) {
        ...
        this.client = accessToken ? strava_v3_1.default.client(accessToken) : strava_v3_1.default;
    }
    /**
    ...
    async refreshAccessToken() {
        ...
        try {
            ...
            this.client = strava_v3_1.default.client(newAccessToken);
     ...
     */
    async updateTokensInEnvFile(accessToken, refreshToken) {
        try {
            ...
            await promises_1.default.writeFile(envPath, newLines.join('\n').trim() + '\n');
     ...
     */
    async handleApiCall(context, apiCall) {
        try {
     ...
     */
    async getActivityStreams(id, types = ['heartrate', 'time', 'distance']) {
     ...
     */
    async getActivitiesByDate(after, before, page = 1, perPage = 30) {
    ...
    async getHeartRateData(activityId) {
        return this.handleApiCall(`getHeartRateData(${activityId})`, async () => {
            ...
            if (heartRateStream && timeStream) {
                combinedData = heartRateStream.data.map((hr, index) => {
                    const dataPoint = {
                        heart_rate: hr,
    ...
    async getRecentActivitiesWithHeartRate(count = 10) {
        return this.handleApiCall(`getRecentActivitiesWithHeartRate(${count})`, async () => {
            ...
            const activities = await this.getActivitiesByDate(thirtyDaysAgo, new Date(), 1, Math.min(count, 100));
            // Filter to only include activities with heart rate data
            const activitiesWithHR = activities.filter((activity) => activity.has_heartrate);
            return {
                total_count: activities.length,
                activities_with_hr: activitiesWithHR.length,
                activities: activitiesWithHR.map((activity) => ({
                    id: activity.id,
                    name: activity.name,
                    type: activity.type,
                    start_date: activity.start_date,
